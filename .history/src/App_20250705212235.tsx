...
const VehicleCard = ({ vehicle }: { vehicle: Vehicle }) => {
  ...
  const statusBgColor = (status: string) => {
    switch (status) {
      case "IN_TRANSIT_TO":
        return 'bg-blue-600/20 text-blue-600 border border-blue-600'
      case "STOPPED_AT":
        return 'bg-yellow-600/20 text-yellow-600 border border-yellow-600'
      case 'INCOMING_AT':
        return 'bg-green-600/20 text-green-600 border border-green-600'
      default:
        return 'bg-gray-600/20 text-gray-600 border border-gray-600'
    }
  }
    ...
    <>
      <div onClick={() => setIsOpen(true)} className='col-span-12 sm:col-span-6 md:col-span-4 lg:col-span-3'>
        <div className='bg-white rounded-lg p-4 shadow-md space-y-3'>
          ...
          <p className='text-sm text-gray-600'>{vehicle.latitude} & {vehicle.longitude}</p>
          <p className='text-sm text-gray-600'>Per {formatDate(vehicle.updated_at)}</p>
        </div>
      </div>
