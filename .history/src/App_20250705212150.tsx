...
const VehicleCard = ({ vehicle }: { vehicle: Vehicle }) => {
  ...
  const statusBgColor = (status: string) => {
    switch (status) {
      case "IN_TRANSIT_TO":
        return 'bg-blue-300/20 text-blue-300 border border-blue-300'
      case "STOPPED_AT":
        return 'bg-yellow-300/20 text-yellow-300 border border-yellow-300'
      case 'INCOMING_AT':
        return 'bg-green-300/20 text-green-300 border border-green-300'
      default:
