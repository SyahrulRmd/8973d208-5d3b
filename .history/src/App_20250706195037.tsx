  ...
  return (
    <div>
      <section className='container min-h-[70dvh]'>
        ...
        <div className='flex flex-col justify-between items-center gap-4 mt-10'>
...
const VehicleCard = ({ vehicle }: { vehicle: TypeWrapper<Vehicle> }) => {
  ...
  return (
    <>
      ...
      <CustomDialog isOpen={isOpen} setIsOpen={setIsOpen} title='Dialog'>
        <div>
          <h3 className='text-lg font-bold'>{vehicle.attributes.label}</h3>
          <p className={`text-sm  ${statusBgColor(vehicle.attributes.current_status)} px-2 py-1 rounded-md`}>{formatStatus(vehicle.attributes.current_status)}</p>
          <p className='text-sm text-gray-500'>{vehicle.attributes.latitude} & {vehicle.attributes.longitude}</p>
          <p className='text-sm text-gray-500'>{formatDate(vehicle.attributes.updated_at)}</p>
        </div>
      </CustomDialog>
    </>
  )
...
const RouteFilter = () => {
  ...
  const selectRoute = (value: string) => {
    console.log(value);
    setSelectedRoute((prev) => [...prev, value])
  }
  ...
  return (
    <div>
      ...
      <div className='flex flex-wrap gap-2'>
        ...
        {routes?.data.length && (
          <Dropdown options={routes?.data?.map((route) => ({ label: route.attributes.long_name, value: route.attributes.short_name }))} selected={selectedRoute[0]} onChange={(value) => selectRoute(value)} />
        )}
      </div>
    </div>
  )
