...
const TripFilter = ({ selectedTrip, setSelectedTrip, selectedRoutes }: {
  ...
  const { data: trips, loadingAddition, refetchAddition, refetch } = useGet<ListWrapper<TypeWrapper<Trip>[]>>(`${BASE_URL}/trips?${encodeURI(pageParams)}`, { isArray: true })
  ...
  useEffect(() => {
    ...
    if (selectedRoutes.length > 0) {
      refetch(`${BASE_URL}/trips?page[offset]=0&page[limit]=10&filter[route]=${selectedRoutes.map((route) => route.id).join(',')}`)
    }
  }, [selectedRoutes, refetch, setSelectedTrip])
