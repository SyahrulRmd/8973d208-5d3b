...
function App() {
  ...
  const pageParams = `page[offset]=${offset}&page[limit]=${limit}&filter[route]=${selectedRoute.join(',')}`
...
const RouteFilter = ({ selectedRoute, setSelectedRoute }: {
  ...
  return (
    <div className='flex flex-wrap gap-2 items-center'>
      <h3 className='font-semibold'>Rute</h3>
      {routes?.data.length && (
        <Listbox value={selectedRoute} onChange={setSelectedRoute} multiple={true}>
          <ListboxButton className={'relative block w-auto rounded-lg bg-white/5 py-1.5 pr-8 pl-3 text-left text-sm border border-gray-500'}>
            {selectedRoute.length > 1 ? `${selectedRoute.length} Terpilih` : selectedRoute.length === 1 ? selectedRoute[0].attributes.short_name : 'Pilih Rute'}
            <ChevronDownIcon
          ...
          <ListboxOptions anchor="bottom" className={'rounded-lg border border-gray-500 bg-white p-1 [--anchor-gap:--spacing(1)] focus:outline-none'}>
            {routes?.data.map((route) => (
              <ListboxOption key={route.id} value={route.id} className="group text-sm flex cursor-default items-center gap-2 rounded-lg px-3 py-1.5 select-none data-focus:bg-white/10">
...
const TripFilter = ({ selectedTrip, setSelectedTrip, selectedRoutes }: {
  ...
  return (
    <div className='flex flex-wrap gap-2 items-center'>
      ...
      {trips?.data.length && (
        <Dropdown
          options={trips?.data?.map((trip) => ({ label: trip.attributes.name, value: trip.id }))}
