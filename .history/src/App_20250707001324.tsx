...
const RouteFilter = ({ selectedRoute, setSelectedRoute }: {
  ...
  const { data: routes, loading, refetchAddition, loadingAddition, refetch } = useGet<ListWrapper<TypeWrapper<Route>[]>>(`${BASE_URL}/routes?${encodeURI(pageParams)}`, { isArray: true })
  ...
  useEffect(() => {
    if (selectedRoute.length > 0) {
      refetch(`${BASE_URL}/routes?${encodeURI(pageParams)}`)
    }
  }, [selectedRoute, pageParams, refetch])
