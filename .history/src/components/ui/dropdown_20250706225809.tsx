...
const Dropdown = <T,>({ options, selected, onChange, isMulti = false, placeholder = 'Pilih' }: DropdownProps<T>) => {
  return (
    <Listbox value={selected} onChange={onChange} multiple={isMulti}>
      <ListboxButton className={'relative block w-auto rounded-lg bg-white/5 py-1.5 pr-8 pl-3 text-left text-sm border border-gray-500'}>
        {typeof selected === 'string' ?
          (selected || placeholder) : Array.isArray(selected) ?
            (selected.length > 1 ? `${selected.length} Terpilih` : selected.length === 1 ? selected[0] : placeholder) :
            placeholder}
        <ChevronDownIcon
