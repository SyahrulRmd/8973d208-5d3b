...
const Button = ({ className, children, variant, ...props }: ButtonProps) => {
  ...
  return (
    <button {...props} className={`transition-all duration-300 px-4 py-2 font-semibold inline-flex gap-2 items-center justify-center rounded-lg ${variantClasses[variant]} ${className}`}>
      {children}
    </button>
  );
}
