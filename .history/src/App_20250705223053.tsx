...
function App() {
  ...
  const { data, loading, error } = useGet<ListWrapper<Vehicle[]>>(`${BASE_URL}/vehicles?${encodeURI(fields)}&${encodeURI(page)}`, { isArray: true },)
        ...
        <div className='grid grid-cols-12 sm:gap-10 gap-4'>
          {loading ? (
            ...
            data?.data?.map((vehicle, index) => (
              <div key={vehicle.label} className='col-span-12 sm:col-span-6 md:col-span-4'>
